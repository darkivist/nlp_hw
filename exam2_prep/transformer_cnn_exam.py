...
vocab = {word: idx + 1 for idx, word in
         enumerate(set(word for review in X_train_tokenized + X_test_tokenized for word in review))}
...
class TransformerCNN(nn.Module):
    def __init__(self, vocab_size, embedding_dim, transformer_dim, num_heads, num_classes):
        ...
        self.conv1d = nn.Conv1d(in_channels=embedding_dim, out_channels=256, kernel_size=3)
        self.fc = nn.Linear(256, num_classes)  # Multi-label classification

    def forward(self, x):
        embedded = self.embedding(x)
        transformer_output = self.transformer(embedded)

        # Permute the dimensions for Conv1d layer (batch_size, embedding_dim, seq_length)
        transformer_output = transformer_output.permute(0, 2, 1)

        cnn_output = self.conv1d(transformer_output)
        cnn_output = nn.functional.relu(cnn_output)
        cnn_output = nn.functional.max_pool1d(cnn_output, kernel_size=cnn_output.size(2)).squeeze(2)
...
model = TransformerCNN(vocab_size, embedding_dim, transformer_dim, num_heads, num_classes)
...
X_test_submission_padded = torch.nn.utils.rnn.pad_sequence(
    [torch.tensor(review) for review in X_test_submission_indices], batch_first=True)
...
test_df['CNN_Computer_Science'] = [pred[0] for pred in predictions]
test_df['CNN_Physics'] = [pred[1] for pred in predictions]
test_df['CNN_Mathematics'] = [pred[2] for pred in predictions]
test_df['CNN_Statistics'] = [pred[3] for pred in predictions]
test_df['CNN_Quantitative_Biology'] = [pred[4] for pred in predictions]
test_df['CNN_Quantitative_Finance'] = [pred[5] for pred in predictions]

# Save the DataFrame with predictions to a new CSV file
test_df.to_csv('CNN_predictions.csv', index=False)

...
train_dataset = torch.utils.data.TensorDataset(X_train_tokenized['input_ids'], X_train_tokenized['attention_mask'], torch.tensor(y_train))
test_dataset = torch.utils.data.TensorDataset(X_test_tokenized['input_ids'], X_test_tokenized['attention_mask'], torch.tensor(y_test))
...
lstm = torch.nn.LSTM(bert_model.config.hidden_size, 512, bidirectional=True, batch_first=True)


class BERT_LSTM(torch.nn.Module):
    def __init__(self, bert_model, lstm):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        lstm_outputs, _ = self.lstm(bert_embeddings)

        # Get LSTM output (e.g., take the last hidden state)
...
# Instantiate the BERT_LSTM model
model = BERT_LSTM(bert_model, lstm)

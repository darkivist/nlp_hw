...
#source: https://www.kaggle.com/code/ksenia5/nltk-categorising-and-tagging-words

brown_tagged_words = [(w.lower(),tag) for (w,tag) in brown.tagged_words()]
#get all singular nouns
all_nouns_sing = [word for (word,tag) in set(brown_tagged_words) if tag=="NN"]

#build dictionary where each noun in singular is counted
all_nouns_plu = [word for (word,tag) in set(brown_tagged_words) if tag=="NNS"]

#find nouns that occur in both singular and plural sets
...
#count frequencioes of singular and plural and add to dictionary
noun_cases = {}
for noun in common_nouns:
    noun_cases[noun] = [sing_noun_fd[noun]]
    noun_cases[noun].append(plu_noun_fd[noun+"s"])

#list words that occur more frequently as plural
common_plurals = [noun for noun in noun_cases if noun_cases[noun][1] > noun_cases[noun][0]]
print("Nouns that appear more frequently as plurals are:", common_plurals)
